describe('Blog app', function () {
  ...
  describe('delete and ordered', function () {
    beforeEach(function () {
      cy.login({ username: 'Anadido', password: 'nuevo' })
      cy.createBlog({ title: ' mas likes', author: 'mayor', url: 'una_tipica_url' })
      cy.contains('view').click()
      cy.get('#like-button').click()
      cy.createBlog({ title: 'creando en before', author: 'autor del before', url: 'una_tipica_url' })
    })
    it('user cannot delete other user blog', function () {
    ...
    it.only('blogs ordered by likes', function () {
      cy.get('.blogshort').eq(0).should('contain', 'mas likes mayor')
      cy.get('.blogshort').eq(1).should('contain', 'creando en before')
      cy.contains('creando en before').find('button').click()
      cy.contains('creando en before').parent().get('#like-button').click()
      cy.wait(100)
      cy.contains('creando en before').parent().get('#like-button').click()
      cy.get('.blogshort').eq(1).should('contain', 'mas likes mayor')
      cy.get('.blogshort').eq(0).should('contain', 'creando en before')
    })
  })
